...
static void GB_flag_interrupt(GB_core_t *gb, GbInterrupt interrupt) {
  gb->io[GB_IO_IF] |= 1 << interrupt;
}

static uint32_t *GBPPU_get_display_screen_buffer(GB_core_t *gb) {
  return gb->current_screen_buffer ? gb->screen_buffer_0 : gb->screen_buffer_1;
}

static uint32_t *GBPPU_get_internal_screen_buffer(GB_core_t *gb) {
  return gb->current_screen_buffer ? gb->screen_buffer_1 : gb->screen_buffer_0;
}

static void GBPPU_swap_buffers(GB_core_t *gb) { gb->current_screen_buffer ^= 1; }

static uint8_t GBPPU_get_mode(GB_core_t *gb) { return gb->io[GB_IO_STAT] & 0b11; }
void GBPPU_set_mode(GB_core_t *gb, GbPpuMode mode) {
  gb->io[GB_IO_STAT] &= ~0b11;
  gb->io[GB_IO_STAT] |= mode;
}

static void GB_write_io_lcdc(GB_core_t *gb, uint8_t val) {
  // bit 7 off to on
...
static void GBPPU_render_scanline(GB_core_t *gb) {
...
static void GB_write_io_if(GB_core_t *gb, uint8_t val) {
  gb->io[GB_IO_IF] = 0b11100000 | val;
}

static uint8_t GB_read_io_joyp(GB_core_t *gb) {
  uint8_t val = gb->io[GB_IO_JOYP] | 0b11001111;

  if (!(gb->io[GB_IO_JOYP] & BIT(5))) {
    if (gb->buttons[BUTTON_START])
      val &= ~BIT(3);
    if (gb->buttons[BUTTON_SELECT])
      val &= ~BIT(2);
    if (gb->buttons[BUTTON_B])
      val &= ~BIT(1);
    if (gb->buttons[BUTTON_A])
      val &= ~BIT(0);
  }
  if (!(gb->io[GB_IO_JOYP] & BIT(4))) {
    if (gb->buttons[BUTTON_DOWN])
      val &= ~BIT(3);
    if (gb->buttons[BUTTON_UP])
      val &= ~BIT(2);
    if (gb->buttons[BUTTON_LEFT])
      val &= ~BIT(1);
    if (gb->buttons[BUTTON_RIGHT])
      val &= ~BIT(0);
  }
...
static void GB_timer_tick(GB_core_t *gb, uint8_t cycles) {
  gb->internal_timer += cycles;
}

static void GB_ppu_tick(GB_core_t *gb, uint8_t cycles) {
...
static uint8_t GB_read_io(GB_core_t *gb, uint16_t addr) {
  switch (addr & 0xFF) {
  case GB_IO_JOYP:
...
static uint8_t GB_read(GB_core_t *gb, uint16_t addr) {
  switch (addr >> 12) {
  case 0x0:
  case 0x1:
  case 0x2:
...
static void GB_write_io(GB_core_t *gb, uint16_t addr, uint8_t val) {
  switch (addr & 0xFF) {
  case GB_IO_IF:
...
static void GB_write(GB_core_t *gb, uint16_t addr, uint8_t val) {
  switch (addr >> 12) {
  case 0x8:
  case 0x9:
...
static void GBCPU_print_status(GB_core_t *gb, uint16_t pc) {
...
static uint16_t GBCPU_get_reg_pair(GB_core_t *gb, uint8_t pair) {
  switch (pair) {
  case GBCPU_REG_BC:
...
static void GBCPU_set_reg_pair(GB_core_t *gb, uint8_t pair, uint16_t val) {
  switch (pair) {
  case GBCPU_REG_BC:
...
static uint16_t GBCPU_get_reg_pair2(GB_core_t *gb, uint8_t pair) {
...
static void GBCPU_set_reg_pair2(GB_core_t *gb, uint8_t pair, uint16_t val) {
  if (pair == GBCPU_REG_SP) {
    gb->sp = val;
  } else {
...
static uint8_t GBCPU_get_reg(GB_core_t *gb, uint8_t reg) {
...
static void GBCPU_set_reg(GB_core_t *gb, uint8_t reg, uint8_t val) {
  if (reg == GBCPU_REG_iHL) {
...
static uint8_t GBCPU_next8(GB_core_t *gb) {
  gb->current_instr_cycles += 4;

  return GB_read(gb, gb->pc++);
}

static uint16_t GBCPU_next16(GB_core_t *gb) {
...
static bool GBCPU_getZ(GB_core_t *gb) { return gb->regs[GBCPU_REG_F] & (1 << 7); }

static bool GBCPU_getN(GB_core_t *gb) { return gb->regs[GBCPU_REG_F] & (1 << 6); }

static bool GBCPU_getH(GB_core_t *gb) { return gb->regs[GBCPU_REG_F] & (1 << 5); }

static bool GBCPU_getC(GB_core_t *gb) { return gb->regs[GBCPU_REG_F] & (1 << 4); }

static void GBCPU_setZ(GB_core_t *gb, bool val) {
  gb->regs[GBCPU_REG_F] &= ~(1 << 7);
  gb->regs[GBCPU_REG_F] |= val << 7;
}

static void GBCPU_setN(GB_core_t *gb, bool val) {
  gb->regs[GBCPU_REG_F] &= ~(1 << 6);
  gb->regs[GBCPU_REG_F] |= val << 6;
}

static void GBCPU_setH(GB_core_t *gb, bool val) {
  gb->regs[GBCPU_REG_F] &= ~(1 << 5);
  gb->regs[GBCPU_REG_F] |= val << 5;
}

static void GBCPU_setC(GB_core_t *gb, bool val) {
  gb->regs[GBCPU_REG_F] &= ~(1 << 4);
  gb->regs[GBCPU_REG_F] |= val << 4;
}

static bool GBCPU_get_cond(GB_core_t *gb, uint8_t cond) {
...
static void GBCPU_stall_cycles(GB_core_t *gb, int32_t cycles) {
  gb->current_instr_cycles += cycles;
}

static uint8_t GBCPU_read(GB_core_t *gb, uint16_t addr) {
  gb->current_instr_cycles += 4;
  return GB_read(gb, addr);
}

static void GBCPU_write(GB_core_t *gb, uint16_t addr, uint8_t val) {
  gb->current_instr_cycles += 4;
  GB_write(gb, addr, val);
}

static void GBCPU_push(GB_core_t *gb, uint16_t val) {
  GBCPU_write(gb, --gb->sp, val >> 8);
  GBCPU_write(gb, --gb->sp, val);
}

static uint16_t GBCPU_pop(GB_core_t *gb) {
...
// ALU ops
static void GBCPU_instr_add(GB_core_t *gb, uint8_t op) {
...
static void GBCPU_instr_adc(GB_core_t *gb, uint8_t op) {
...
static void GBCPU_instr_sub(GB_core_t *gb, uint8_t op) {
  uint8_t res = gb->regs[GBCPU_REG_A] - op;
...
static void GBCPU_instr_sbc(GB_core_t *gb, uint8_t op) {
  uint8_t res = gb->regs[GBCPU_REG_A] - op - GBCPU_getC(gb);
...
static void GBCPU_instr_and(GB_core_t *gb, uint8_t op) {
  uint8_t res = gb->regs[GBCPU_REG_A] & op;
...
static void GBCPU_instr_xor(GB_core_t *gb, uint8_t op) {
  uint8_t res = gb->regs[GBCPU_REG_A] ^ op;
...
static void GBCPU_instr_or(GB_core_t *gb, uint8_t op) {
  uint8_t res = gb->regs[GBCPU_REG_A] | op;
...
static void GBCPU_instr_cp(GB_core_t *gb, uint8_t op) {
  uint8_t res = gb->regs[GBCPU_REG_A] - op;
...
static uint8_t GBCPU_execute(GB_core_t *gb) {
...
void GB_set_button(GB_core_t *gb, GbButtons button, bool state) {
  gb->buttons[button] = state;
}
